...
CREATE TABLE IF NOT EXISTS `DMS`.`ORGANIZATION` (
  `ORGANIZATION_ID` BIGINT(10) NOT NULL COMMENT 'This table maintains the dace information of organization / Company / Individual',
...
CREATE TABLE IF NOT EXISTS `DMS`.`PRODUCT` (
  ...
  CONSTRAINT `fk_PRODUCT_ORGANIZATION1`
    ...
    REFERENCES `DMS`.`ORGANIZATION` (`ORGANIZATION_ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `DMS`.`PRODUCT_DOC_CONF` (
  ...
  `PRODUCT_DOC_CONFcol` VARCHAR(45) NULL,
